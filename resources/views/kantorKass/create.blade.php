            ...
            <div class="card">
                <div class="card-body">
                    <div class="col-12 mb-3">
                        <h3 align="center">Tambah Kas</h3>
                    </div>
                    <form action="{{ route('kantor_kas.store', $kantorCabang->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group">
                            <label for="nama" class="font-weight-bold">Nama Kas</label>
                            <input type="text" id="nama" class="form-control {{ $errors->first('nama') ? 'is-invalid' : '' }}" name="nama" placeholder="Nama Kas" value="{{ old('nama') }}" required>
                        ...
                        <div class="form-group mt-4">
                            <a href="{{ route('kantor_kas.index', $kantorCabang->id) }}" class="btn btn-secondary">Kembali</a>
...
@section('script')
    ...
    <script>
        CKEDITOR.replace('keterangan', {
            filebrowserUploadUrl    : "{{ route('kantor_kas.store' , $kantorCabang->id, ['_token' => csrf_token()]) }}",
